...
class TestRSSMonitor:
    ...
    async def test_process_feed_success(self, mock_changed, mock_healthy, mock_should_poll, mock_fetch):
    ...
    @patch('ozb_deal_filter.components.rss_monitor.FeedPoller.should_poll')
    @patch('ozb_deal_filter.components.rss_monitor.FeedPoller.is_healthy')
    async def test_process_feed_fetch_failure(self, mock_healthy, mock_should_poll, mock_fetch):
