...
class TestApplicationOrchestrator:
    ...
    async def test_initialization_success(self, config_file):
        ...
        with patch(
            "ozb_deal_filter.orchestrator.ConfigurationManager"
        ) as mock_config_mgr, patch(
            "ozb_deal_filter.orchestrator.RSSMonitor"
        ) as mock_rss, patch(
            "ozb_deal_filter.orchestrator.DealParser"
        ) as mock_parser, patch(
            "ozb_deal_filter.orchestrator.LLMEvaluator"
        ) as mock_llm, patch(
            "ozb_deal_filter.orchestrator.AlertFormatter"
        ) as mock_formatter, patch(
            "ozb_deal_filter.orchestrator.MessageDispatcherFactory"
        ) as mock_dispatcher_factory, patch(
            "ozb_deal_filter.orchestrator.EvaluationService"
            ...
            mock_dispatcher_factory.create_dispatcher.return_value = (
            ...
            mock_eval_service_instance.evaluate_deal = AsyncMock(
                return_value=EvaluationResult(
                    is_relevant=True, confidence_score=0.8, reasoning="Test evaluation"
                )
            )
    ...
    @pytest.mark.asyncio
    async def test_process_single_deal_success(
        self, config_file, sample_raw_deal, sample_deal
    ):
        ...
        with patch(
            ...
            "ozb_deal_filter.orchestrator.EvaluationService"
        ) as mock_eval_service, patch(
            "ozb_deal_filter.orchestrator.AlertFormatter"
        ) as mock_formatter, patch(
            "ozb_deal_filter.orchestrator.MessageDispatcher"
        ) as mock_dispatcher:
            # Setup mocks
            mock_config_instance = Mock()
            mock_config_mgr.return_value = mock_config_instance
            ...
            mock_dispatcher_instance.send_alert = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_process_single_deal_llm_fallback(
        self, config_file, sample_raw_deal, sample_deal
    ):
    ...
    async def test_apply_filters_pass(self, config_file, sample_deal):
        ...
        evaluation = EvaluationResult(
            is_relevant=True, confidence_score=0.8, reasoning="Test"
        )
