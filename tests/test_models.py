...
class TestUserCriteria:
    ...
    def test_invalid_authenticity_score_raises_error(self):
        ...
        with pytest.raises(
            ValueError, match="Minimum authenticity score must be between 0 and 1"
        ):
            criteria.validate()
...
class TestLLMProviderConfig:
    ...
    def test_invalid_type_raises_error(self):
        ...
        with pytest.raises(
            ValueError, match="LLM provider type must be 'local' or 'api'"
        ):
            config.validate()
...
class TestMessagingPlatformConfig:
    ...
    def test_invalid_discord_webhook_raises_error(self):
        ...
        config = MessagingPlatformConfig(
            type="discord", discord={"webhook_url": "https://invalid.com/webhook"}
        )
...
class TestFilterResult:
    ...
    def test_invalid_authenticity_score_raises_error(self):
        ...
        with pytest.raises(
            ValueError, match="authenticity_score must be between 0 and 1"
        ):
            result.validate()
...
class TestDeliveryResult:
    ...
    def test_valid_successful_delivery(self):
        ...
        result = DeliveryResult(
            success=True, delivery_time=datetime.now(), error_message=None
        )
        assert result.validate() is True
    ...
    def test_valid_failed_delivery(self):
        ...
        result = DeliveryResult(
            success=False, delivery_time=datetime.now(), error_message="Network timeout"
        )
        assert result.validate() is True
    ...
    def test_failed_delivery_without_error_message_raises_error(self):
        ...
        with pytest.raises(
            ValueError, match="error_message should be provided when success is False"
        ):
            result.validate()
