...
def print_test_result(result: TestResult):
    ...
    print_colored(f"  {result.test_name}: {status_text} ({format_duration(result.duration)})", status_color)
    ...
    if result.success and result.response_data:
        if "models" in result.response_data:
            ...
            print_colored(f"    Successful: {data['successful_requests']}/{data['total_requests']}", "blue")
            print_colored(f"    Avg Response Time: {format_duration(data['average_response_time'])}", "blue")
...
def main():
    ...
    if not health_result.success:
        ...
        print_colored("  1. Docker containers are running: docker-compose up -d", "yellow")
    ...
    if passed == total:
        print_colored(f"All {total} tests passed! LLM integration is working correctly.", "green")
    else:
        print_colored(f"{passed}/{total} tests passed. Please check failed tests above.", "yellow")
    
    print_colored("\nRecommendations:", "blue")
    print_colored("  - For best performance, use llama2:7b or mistral:7b models", "yellow")
    print_colored("  - Monitor response times and adjust timeout settings if needed", "yellow")
