...
class ConfigurationManager(IConfigurationManager):
    ...
    def _find_config_file(self) -> str:
        ...
        if os.path.exists("config/config.example.yaml"):
            raise ValueError(
                "No configuration file found. Please copy "
                "'config/config.example.yaml' to 'config/config.yaml' and "
    ...
    def validate_config_file(self, config_path: str) -> bool:
        ...
        try:
            ...
            # Expand environment variables (but don't fail on missing ones
            # for validation)
            try:
                raw_config = self._expand_env_vars(raw_config)
            except ValueError:
                # For validation, we'll skip env var expansion if vars are
                # missing
                pass
    ...
    def load_configuration(self, config_path: str) -> Configuration:
        """
        Load configuration from file (protocol interface method).
        ...
        Returns:
            Configuration object with validated settings.
        """
        # Update the config path and load
        self.config_path = config_path
        return self.load_config()

    def validate_configuration(self, config: Configuration) -> bool:
        """
        Validate configuration data (protocol interface method).
    ...
    def reload_configuration(self) -> None:
        """
        Reload configuration without restart (protocol interface method).
        """
