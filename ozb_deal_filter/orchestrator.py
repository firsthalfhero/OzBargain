...
class ApplicationOrchestrator:
    ...
    async def _initialize_components(self) -> bool:
        ...
        try:
            ...
            self._message_dispatcher = MessageDispatcherFactory.create_dispatcher(
                self._config.messaging_platform.type,
                platform_config
            )
        ...
        try:
            ...
            for component in critical_components:
                if not self._component_health.get(component, False):
                    self.logger.error(f"Critical component '{component}' is not healthy")
                    return False
        ...
        try:
            ...
            if new_config.messaging_platform != self._config.messaging_platform:
                ...
                try:
                    ...
                    self._message_dispatcher = MessageDispatcherFactory.create_dispatcher(
                        new_config.messaging_platform.type,
                        platform_config
                    )
                    self._component_health["message_dispatcher"] = True
    ...
    async def _process_single_deal(self, raw_deal: RawDeal) -> None:
        ...
        if not evaluation_result.is_relevant:
            self.logger.debug(
                "Deal not relevant",
                extra={"deal_title": deal.title, "reasoning": evaluation_result.reasoning}
            )
            return
    ...
    async def _apply_filters(self, deal: Deal, evaluation: EvaluationResult) -> FilterResult:
        ...
        if (self._config.user_criteria.min_discount_percentage is not None 
            and deal.discount_percentage is not None):
            passes_discount = deal.discount_percentage >= self._config.user_criteria.min_discount_percentage
        ...
        passes_authenticity = authenticity_score >= self._config.user_criteria.min_authenticity_score
        ...
        elif "limited time" in deal.title.lower() or "expires" in deal.description.lower():
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "uptime": str(datetime.now() - self._startup_time) if self._startup_time else None,
