...
from ..interfaces import IDealParser
...
class PriceExtractor:
    ...
    PRICE_PATTERNS = [
        ...
        r'(?:price|cost|now|sale|deal)\s*:?\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)',
    ]
    ...
    ORIGINAL_PRICE_PATTERNS = [
        ...
        r'(?:was|originally|rrp|retail)\s*:?\s*\$?(\d{1,4}(?:,\d{3})*(?:\.\d{2})?)',
    ...
    def __init__(self):
        ...
        self.price_regexes = [re.compile(pattern, re.IGNORECASE) for pattern in self.PRICE_PATTERNS]
        self.discount_regexes = [re.compile(pattern, re.IGNORECASE) for pattern in self.DISCOUNT_PATTERNS]
        self.original_price_regexes = [re.compile(pattern, re.IGNORECASE) for pattern in self.ORIGINAL_PRICE_PATTERNS]
        self.urgency_regexes = [re.compile(pattern, re.IGNORECASE) for pattern in self.URGENCY_PATTERNS]
    ...
    def extract_discount_percentage(self, text: str, current_price: Optional[float] = None, original_price: Optional[float] = None) -> Optional[float]:
        """
...
class DealValidator:
    ...
    def __init__(self):
        ...
        self.required_fields = ['id', 'title', 'description', 'category', 'url', 'timestamp']
    
    def validate_deal(self, deal: Deal) -> bool:
        """
        Validate that a deal contains required information and is logically consistent.
    ...
    def _validate_price_logic(self, deal: Deal) -> None:
        ...
        if (deal.discount_percentage is not None and 
            deal.price is not None and 
            deal.original_price is not None and 
            deal.original_price > 0):
            
            calculated_discount = ((deal.original_price - deal.price) / deal.original_price) * 100
            
            # Allow some tolerance for rounding differences
            if abs(calculated_discount - deal.discount_percentage) > 5:
                logger.warning(
                    f"Discount percentage mismatch for deal {deal.id}: "
                    f"stated {deal.discount_percentage}%, calculated {calculated_discount:.1f}%"
                )
...
class DealParser:
    ...
    def parse_deal(self, raw_deal: RawDeal) -> Deal:
        ...
        try:
            ...
            current_price, original_price = self.price_extractor.extract_prices(combined_text)
            discount_percentage = self.price_extractor.extract_discount_percentage(
                combined_text, current_price, original_price
            )
            
            # Extract urgency indicators
            urgency_indicators = self.price_extractor.extract_urgency_indicators(combined_text)
    ...
    def _extract_community_data(self, raw_deal: RawDeal) -> Tuple[Optional[int], Optional[int]]:
        """
