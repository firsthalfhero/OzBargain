...
from ..interfaces import IRSSMonitor, IDealDetector
...
class FeedPoller:
    ...
    def fetch_feed(self) -> Optional[str]:
        ...
        try:
            ...
            self.consecutive_failures += 1
            logger.error(
                f"HTTP error {e.response.status_code} for feed {self.feed_url} "
                f"(failure #{self.consecutive_failures})"
            )
            return None
...
class DealDetector:
    ...
    def detect_new_deals(self, feed_data: str) -> List[RawDeal]:
        ...
        try:
            ...
            if parsed_feed.bozo:
                logger.warning(f"RSS feed parsing warning: {parsed_feed.bozo_exception}")
            
            new_deals = []
            
            for entry in parsed_feed.entries:
                try:
                    # Create unique ID for deal (using link as primary identifier)
    ...
    async def _monitor_loop(self) -> None:
        ...
        while self.is_monitoring:
            try:
                ...
                await asyncio.sleep(10)  # Check every 10 seconds for feeds ready to poll
